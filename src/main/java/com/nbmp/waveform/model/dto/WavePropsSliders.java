...
public class WavePropsSliders {
    ...
    public void addListenerAccordingToTarget(Target target) {
        switch (target) {
            case FREQUENCY -> addListener(slider, "Frequency", label, (newValue) -> {
                waveform.getWaveform().getProps().setFrequency(newValue);
                updateTask.accept(newValue);
            });
            case AMPLITUDE -> addListener(slider, "Amplitude", label, (newValue) -> {
                waveform.getWaveform().getProps().setAmplitude(newValue);
                updateTask.accept(newValue);
            });
            case PHASE -> addListener(slider, "Phase", label, (newValue) -> {
    ...
    public void addListener(
            Slider controlSlider, String labelText, Label labelOfAffectedSlider, Consumer<Double> updateFunction) {
        PauseTransition pause = new PauseTransition(Duration.millis(50));
        controlSlider
                .valueProperty()
                .addListener(
                        (observable, oldValue, newValue) -> {
                            labelOfAffectedSlider.setText("%s: %.2f Hz".formatted(labelText, newValue.doubleValue()));
                            pause.setOnFinished(
                                    event -> {
